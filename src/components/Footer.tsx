...
import { MENU, NAME, SECTIONS_VISIBILITY, SOCIAL_LINKS } from "../data/data";
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <nav aria-label="Footer navigation">
          <ul className="flex flex-wrap justify-center gap-x-6 gap-y-2">
            {MENU.filter(({ route }) => SECTIONS_VISIBILITY[route as keyof typeof SECTIONS_VISIBILITY]).map(({ key, name, route }) => (
              <li key={key}>
                <Link
                  to={route}
                  spy={true}
